...
def make_piecewise_constant_ic(
    x: np.ndarray,
    ...
    breakpoints: [b1, b2, ..., bk]
    values:      [v0, v1, ..., vk], length = k+1
    """
    if len(values) != len(breakpoints) + 1:
        raise ValueError("Need len(values) = len(breakpoints) + 1")

    u0 = np.empty_like(x, dtype=np.float32)
    left = -np.inf
    for j, bp in enumerate(breakpoints):
        mask = (x >= left) & (x < bp)
        u0[mask] = values[j]
        left = bp
    u0[x >= breakpoints[-1]] = values[-1]
    return u0.astype(np.float32)

def make_sinusoid_ic(
    x: np.ndarray,
    mean: float = 0.55,
    ...
    """
    Sinusoidal initial condition optionally quantized into
    piecewise-constant levels.

    u(x) = mean + amplitude * sin(2πk x)
    """

    u = mean + amplitude * np.sin(2.0 * np.pi * frequency * x)
...
def solve_truth_fixed_dt(
    ...
    with fixed dt so all ICs produce the same nt.
    """
...
class FluxGNN1DLatent(nn.Module):
    def __init__(
        ...
        base_flux_weight: float = 0.10,
        latent_flux_scale: float = 0.05,
    ) -> None:
        super().__init__()
    ...
    def decode_clamped(self, h: torch.Tensor) -> torch.Tensor:
        return torch.clamp(self.decode(h), 0.0, 1.0)
    ...
    def step(self, u: torch.Tensor, dt: float, dx: float, boundary: str = "copy") -> torch.Tensor:
        ...
        if boundary == "copy":
            ...
            return self.decode_clamped(h_new)
        ...
        if boundary == "fixed":
            ...
            u_new = self.decode_clamped(h_new)
            u_new[:, 0] = u[:, 0]
            u_new[:, -1] = u[:, -1]
            return u_new
...
class Config:
    ...
    # For concave LWR flux f(u)=u(1-u):
    #   uL > uR  => rarefaction
    #   uL < uR  => shock
    ...
    # 4 interfaces -> alternating rarefactions and shocks
    # 0.78->0.18 rarefaction
    # 0.18->0.82 shock
    # 0.82->0.30 rarefaction
    # 0.30->0.76 shock
    test_breakpoints: Tuple[float, ...] = (-0.60, -0.05, 0.35, 0.70)
    test_values: Tuple[float, ...] = (0.78, 0.18, 0.82, 0.30, 0.76)
    ...
    save_dir: str = "results_fluxgnn_multi_jump"
...
def build_grid(cfg: Config) -> Tuple[np.ndarray, float]:
    ...
    dx = float(x[1] - x[0])   # match gudonov.py
    return x, dx
...
def build_train_dataset(
    ...
    for uL, uR in cfg.train_pairs:
        ...
        if times_ref is None:
            ...
            assert len(times) == len(times_ref), "Time grids differ."
...
def build_multi_jump_test_ic(cfg: Config, x: np.ndarray) -> np.ndarray:
    return make_piecewise_constant_ic(
        x=x,
        breakpoints=list(cfg.test_breakpoints),
...
# ============================================================
# 9) Evaluation on multi-jump IC
# ============================================================
def evaluate_multi_jump(
    cfg: Config,
    model: FluxGNN1DLatent,
    x: np.ndarray,
    dx: float,
    ...
    u0_test = build_multi_jump_test_ic(cfg, x)
    ...
    print("\nMulti-jump test")
    print(f"  breakpoints = {cfg.test_breakpoints}")
    print(f"  values      = {cfg.test_values}")
    print(f"  MSE         = {mse:.8e}")
    print(f"  MAE         = {mae:.8e}")
...
def evaluate_sinusoid(
    ...
    """
    Test generalization on sinusoidal IC producing
    many shocks and rarefactions.
    """

    u0_test = make_sinusoid_ic(
        x,
        mean=0.55,
        amplitude=0.28,
        frequency=2,
...
def plot_test_initial_condition(cfg: Config, x, u0_test):
    plt.figure(figsize=(8, 4))
    plt.plot(x, u0_test, linewidth=2)
    plt.xlabel("x")
    plt.ylabel(r"$u_0(x)$")
    plt.title("Multi-jump test initial condition")
    plt.grid(alpha=0.25)
    plt.tight_layout()
    plt.savefig(os.path.join(cfg.save_dir, "test_initial_condition.png"), dpi=160)
    plt.close()
...
def plot_final_snapshot(cfg: Config, x, U_true, U_pred):
    ...
    plt.title("Multi-jump: final snapshot")
    plt.legend()
    plt.tight_layout()
...
def plot_final_error(cfg: Config, x, err):
    ...
    plt.title("Multi-jump: final error")
    plt.tight_layout()
...
def plot_space_time(cfg: Config, x, times, U_true, U_pred, err):
    ...
    im0 = axes[0].imshow(
        U_true[:nt], aspect="auto", extent=extent, origin="lower",
    ...
    im1 = axes[1].imshow(
        U_pred[:nt], aspect="auto", extent=extent, origin="lower",
        cmap="viridis", vmin=0.0, vmax=1.0
    )
    ...
    vmax_err = max(1e-8, min(0.5, float(np.max(np.abs(err[:nt])))))
    im2 = axes[2].imshow(
        np.abs(err[:nt]), aspect="auto", extent=extent, origin="lower",
        cmap="hot", vmin=0.0, vmax=vmax_err
    )
...
def main():
    ...
    print(f"  test breaks   : {cfg.test_breakpoints}")
    print(f"  test values   : {cfg.test_values}")
    ...
    with torch.no_grad():
        e = model.encoder_vec
        d = model.decoder_vec()
        print(f"Init check: d^T e = {torch.dot(d, e).item():.8f}")
    ...
    with torch.no_grad():
        e = model.encoder_vec
        d = model.decoder_vec()
        print(f"Post-train: d^T e = {torch.dot(d, e).item():.8f}")

    torch.save(model.state_dict(), os.path.join(cfg.save_dir, "fluxgnn1d_latent_model.pt"))

    u0_test, U_true, U_pred, err, times_test, mse, mae, linf = evaluate_sinusoid(cfg, model, x, dx, dt)
    ...
    np.savez_compressed(
        os.path.join(cfg.save_dir, "multi_jump_results.npz"),
    ...
    with open(os.path.join(cfg.save_dir, "metrics.txt"), "w") as f:
        f.write("Train on few single-jump Riemann problems, test on multi-jump IC\n")
        f.write(f"train_pairs = {cfg.train_pairs}\n")
        f.write(f"test_breakpoints = {cfg.test_breakpoints}\n")
        f.write(f"test_values = {cfg.test_values}\n")
    ...
    plot_test_initial_condition(cfg, x, u0_test)
